...
struct StubNetworkClient: NetworkRouting {
    ...
    func fetch(url: URL, handler: @escaping (Result<Data, Error>) -> Void) {
        if emulateError {
            ...
            handler(.success(expectedResponsse))
        }
        }
    private var expectedResponsse: Data {
        """
                {
                   "errorMessage" : "",
                   "items" : [
...
class MoviesLoaderTests: XCTestCase {
    ...
    func testFailureLoading() throws {
        //given
        let stubNetworkClient = StubNetworkClient(emulateError: true) // тут эмулируем ошибку
        //when
        let loader = MoviesLoader(networkClient: stubNetworkClient)
        
        let expetation = expectation(description: "Loading expectation")
